 ...
 * @param {{ lignes:{label:string, valeur:number, avant?:number, couleur:string}[], formatValeur:(v:number)=>string, max?:number, libelleAvant?:string }} config
 * @returns {{ definirApres:(valeurs:(number|null)[]|null)=>void }}
 */
export function barresHorizontales(zone, config) {
  let apres = null;
  const dessiner = () => {
    const { lignes, formatValeur, libelleAvant = "1996" } = config;
    ...
    zone.replaceChildren(
      h(
        ...
        lignes.map((l, i) => {
          const a = apres?.[i];
          const texte = [l.label, formatValeur(l.valeur), l.avant != null ? `en ${libelleAvant} : ${formatValeur(l.avant)}` : null, a != null ? `avec vos réformes : ${formatValeur(a)}` : null]
            .filter(Boolean)
            .join(", ");
          return h(
            "li",
            ...
            h(
              ...
              l.avant != null ? h("span", { class: "barre__repere barre__repere--avant", style: { left: pct(l.avant) }, title: `${libelleAvant} : ${formatValeur(l.avant)}` }) : null,
